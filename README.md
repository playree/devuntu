...
カレンダー連携・メール/Slack/Webプッシュ通知・MCP/AIエージェント連携などを備えた
...
- AIエージェントと連携し、自動的にチケットをAIエージェントで対応できる
- パスキー認証、Googleログイン
- OAuthプロバイダとして利用できる
...
- **エージェントに任せる** — エージェントを担当者にし、チケットの「エージェントモード」を選ぶと、
  利用者のマシンで AIエージェントのCLI が自動起動して対応します。プランを先に投稿させることもできます。
  仕組みは [docs/agent-runner.md](docs/agent-runner.md) を参照
- **リモート実行** — あらかじめ定義した処理をリモートのサーバーで実行し、出力をリアルタイムで確認
  (既定は無効)。仕組みと定義の書き方は [docs/command-exec.md](docs/command-exec.md) を参照
