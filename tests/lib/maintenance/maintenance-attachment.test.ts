...
describe('参照の判定', () => {
  it('集合に載っているキーは消さない(再確認も引かない)', async () => {
    ...
    vi.mocked(prisma.attachment.findMany).mockResolvedValue([orphan])
...
describe('削除の順序', () => {
  ...
  it('1件の失敗で残りを止めない', async () => {
    vi.mocked(prisma.attachment.findMany).mockResolvedValue([orphan, { id: 'a2', key: 'other.webp', size: 1 }])
...
describe('上限と実行間隔', () => {
  it('1周の削除件数を上限で打ち切る', async () => {
    ...
    vi.mocked(prisma.attachment.findMany).mockResolvedValue(many)
