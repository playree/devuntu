...
※シークレットは後から再確認できません。`,
  ...
  msg_agent_mode_desc:
    'エージェントに任せる方式を選びます。プラン先行はプランを投稿して返信を待ち、自動実行はそのまま対応して報告します。',
  msg_agent_runner_desc:
    '担当チケットを定期的に取りに来る常駐エージェントの設定です。稼働条件を満たすときだけ Claude が起動します。',
  msg_agent_token_once: 'トークンを確認できるのはこの画面だけです。閉じる前に控えてください。',
  msg_agent_window_desc:
    '開始と終了のどちらかが未指定なら終日稼働します。開始が終了より後の場合は日跨ぎ(夜間のみ)として扱います。',
