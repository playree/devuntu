// アルファベット順
export type LocaleItemBase =
  | 'account'
  | 'account_connect_google'
  | 'account_connect_slack'
  | 'account_disconnect'
  | 'account_relink'
  | 'action'
  | 'add_agent'
  | 'add_agent_approver_user'
  | 'add_board'
  | 'add_busy_time'
  | 'add_client'
  | 'add_comment'
  | 'add_group'
  | 'add_link'
  | 'add_member'
  | 'add_repository'
  | 'add_tag'
  | 'add_ticket'
  | 'add_user'
  | 'admin'
  | 'admin_regist'
  | 'agent'
  | 'agent_action_execute'
  | 'agent_action_plan'
  | 'agent_action_revise'
  | 'agent_approvals'
  | 'agent_approvals_total'
  | 'agent_approver'
  | 'agent_approver_group'
  | 'agent_custom_instruction'
  | 'agent_daily_limit'
  | 'agent_daily_reset'
  | 'agent_daily_usage'
  | 'agent_duration'
  | 'agent_handle'
  | 'agent_host'
  | 'agent_last_polled'
  | 'agent_manage'
  | 'agent_mode'
  | 'agent_mode_auto'
  | 'agent_mode_none'
  | 'agent_mode_plan'
  | 'agent_poll_interval'
  | 'agent_profile'
  | 'agent_rule'
  | 'agent_run_failed'
  | 'agent_run_history'
  | 'agent_run_running'
  | 'agent_run_skipped'
  | 'agent_run_succeeded'
  | 'agent_runner'
  | 'agent_runner_disabled'
  | 'agent_runner_none'
  | 'agent_runner_offline'
  | 'agent_runner_online'
  | 'agent_runs_recent'
  | 'agent_settings'
  | 'agent_started_at'
  | 'agent_state'
  | 'agent_state_done'
  | 'agent_state_failed'
  | 'agent_state_planned'
  | 'agent_state_queued'
  | 'agent_state_running'
  | 'agent_state_skipped'
  | 'agent_token'
  | 'agent_unlimited'
  | 'all'
  | 'allow'
  | 'announcement'
  | 'announcement_edit'
  | 'announcement_manage'
  | 'app_info'
  | 'archive'
  | 'archived'
  | 'assignee'
  | 'auth'
  | 'auth_ng'
  | 'authenticator'
  | 'avatar'
  | 'back'
  | 'board'
  | 'board_github'
  | 'board_groups'
  | 'board_key'
  | 'board_members'
  | 'board_notify_event_agent_run'
  | 'board_notify_event_ticket_assigned'
  | 'board_notify_event_ticket_completed'
  | 'board_notify_event_ticket_created'
  | 'board_profile'
  | 'board_settings'
  | 'board_slack_notify'
  | 'buildno'
  | 'busy'
  | 'busy_time_manage'
  | 'calendar'
  | 'calendar_share'
  | 'calendar_share_default_title'
  | 'cancel'
  | 'ci_status_cancelled'
  | 'ci_status_failure'
  | 'ci_status_pending'
  | 'ci_status_success'
  | 'claude_code'
  | 'clear'
  | 'client_id'
  | 'client_name'
  | 'client_secret'
  | 'close'
  | 'codex_cli'
  | 'command_confirm_required'
  | 'command_def_add'
  | 'command_def_conflict'
  | 'command_def_delete'
  | 'command_def_delete_confirm'
  | 'command_def_dir'
  | 'command_def_edit'
  | 'command_def_loaded'
  | 'command_def_not_editable'
  | 'command_def_read_only'
  | 'command_def_reload'
  | 'command_def_template_label'
  | 'command_def_yaml'
  | 'command_def_yaml_invalid'
  | 'command_definition'
  | 'command_disabled'
  | 'command_exec'
  | 'command_executed_by'
  | 'command_exit_code'
  | 'command_follow_latest'
  | 'command_fresh_session_required'
  | 'command_input_type_checkbox'
  | 'command_input_type_input'
  | 'command_input_type_multiselect'
  | 'command_input_type_radio'
  | 'command_input_type_select'
  | 'command_inputs'
  | 'command_invalid_def'
  | 'command_manage'
  | 'command_no_def'
  | 'command_no_input'
  | 'command_no_target'
  | 'command_reconnecting'
  | 'command_run'
  | 'command_run_args'
  | 'command_run_cancel'
  | 'command_run_detail'
  | 'command_run_finished_at'
  | 'command_run_history'
  | 'command_run_log'
  | 'command_run_queued_at'
  | 'command_run_scope_all'
  | 'command_runs_recent'
  | 'command_singleton'
  | 'command_status_canceled'
  | 'command_status_failed'
  | 'command_status_queued'
  | 'command_status_running'
  | 'command_status_succeeded'
  | 'command_sys_args_mismatch'
  | 'command_sys_canceled'
  | 'command_sys_connection_lost'
  | 'command_sys_def_missing'
  | 'command_sys_interrupted'
  | 'command_sys_log_partial'
  | 'command_sys_log_write_failed'
  | 'command_sys_output_limit'
  | 'command_sys_queued_canceled'
  | 'command_sys_ssh_error'
  | 'command_sys_ssh_setup_failed'
  | 'command_sys_timeout'
  | 'command_target'
  | 'command_target_assign'
  | 'command_target_editable'
  | 'command_target_groups'
  | 'command_target_identity'
  | 'command_target_known_hosts'
  | 'command_target_members'
  | 'command_target_orphan'
  | 'command_target_orphan_description'
  | 'command_target_orphan_unknown'
  | 'command_target_purge'
  | 'command_target_settings'
  | 'command_timeout'
  | 'command_truncated'
  | 'comment'
  | 'comment_type'
  | 'comment_type_none'
  | 'comment_type_plan'
  | 'comment_type_report'
  | 'completed_at'
  | 'completed_within'
  | 'confirm_archive'
  | 'confirm_deletion'
  | 'confirmed'
  | 'consent'
  | 'consent_apps'
  | 'content'
  | 'copied'
  | 'copy'
  | 'copy_url'
  | 'create_tag'
  | 'created_at'
  | 'current_token'
  | 'danger_zone'
  | 'dashboard'
  | 'dashboard_manage'
  | 'default_layout_manage'
  | 'delete'
  | 'delete_board'
  | 'delete_image'
  | 'description'
  | 'direct'
  | 'disable'
  | 'disable_sharing'
  | 'disabled'
  | 'display_order'
  | 'due_date'
  | 'due_soon'
  | 'edit_dashboard'
  | 'edit_link'
  | 'email'
  | 'enable'
  | 'enable_sharing'
  | 'enabled'
  | 'end_time'
  | 'error'
  | 'everyday'
  | 'expires_in_days'
  | 'filter'
  | 'free_memory'
  | 'github_complete_on_pr_merge'
  | 'github_repositories'
  | 'github_repository'
  | 'github_webhook_url'
  | 'google_account'
  | 'google_account_allowed_groups'
  | 'google_account_enable'
  | 'google_signin'
  | 'granted_at'
  | 'granted_scopes'
  | 'group'
  | 'group_manage'
  | 'groups'
  | 'hide'
  | 'icon'
  | 'id'
  | 'image_actual_size'
  | 'image_fit_screen'
  | 'immutable'
  | 'insert_image'
  | 'integration_settings'
  | 'is_admin'
  | 'issue_token'
  | 'issued_at'
  | 'issuer_url'
  | 'kanban'
  | 'keyword'
  | 'last_login'
  | 'last_sent'
  | 'last_used'
  | 'link_text'
  | 'link_widget_manage'
  | 'linode_transfer_info'
  | 'maintenance_mode'
  | 'mcp_add_command'
  | 'mcp_token'
  | 'mcp_token_env_command'
  | 'member'
  | 'mention_in_ticket'
  | 'mentioned'
  | 'msg_board_github_desc'
  | 'msg_command_already_running'
  | 'msg_command_cancel_confirm'
  | 'msg_command_cancel_requested'
  | 'msg_command_confirm'
  | 'msg_command_group_is_member'
  | 'msg_command_owner_can_edit'
  | 'msg_command_queue_full'
  | 'msg_github_complete_on_pr_merge'
  | 'msg_maintenance_mode'
  | 'my_mentions'
  | 'my_tickets'
  | 'name'
  | 'name_locked'
  | 'next'
  | 'next_week'
  | 'no_expiration'
  | 'no_file_selected'
  | 'no_name'
  | 'no_tag_selected'
  | 'not_found'
  | 'not_issued'
  | 'not_ready'
  | 'not_selected'
  | 'notify_channel_email'
  | 'notify_channel_slack'
  | 'notify_channel_webpush'
  | 'notify_event_agent_run'
  | 'notify_event_mention'
  | 'notify_event_ticket_assigned'
  | 'notify_event_ticket_completed'
  | 'notify_event_ticket_created'
  | 'notify_msg_ticket_completed_by_pr'
  | 'notify_settings'
  | 'notify_webpush'
  | 'notify_webpush_devices'
  | 'notify_webpush_disable'
  | 'notify_webpush_enable'
  | 'notify_webpush_this_device'
  | 'oidc_clients'
  | 'oidc_clients_dynamic'
  | 'oidc_clients_manual'
  | 'ok'
  | 'otp'
  | 'overdue'
  | 'owner'
  | 'passkey'
  | 'passkey_signin'
  | 'password'
  | 'password_score'
  | 'password_score_required'
  | 'per_page'
  | 'pr_state_closed'
  | 'pr_state_draft'
  | 'pr_state_merged'
  | 'pr_state_open'
  | 'prev'
  | 'prev_week'
  | 'preview'
  | 'priority'
  | 'priority_high'
  | 'priority_low'
  | 'priority_medium'
  | 'priority_urgent'
  | 'privacy_policy'
  | 'private'
  | 're_auth'
  | 'ready'
  | 'recent_activity'
  | 'redirect_uri'
  | 'regenerate_url'
  | 'register_passkey'
  | 'registered_at'
  | 'reissue_token'
  | 'release_note'
  | 'reload'
  | 'remove_link'
  | 'remove_target'
  | 'replace_image'
  | 'reply'
  | 'require_pkce'
  | 'resend'
  | 'results_none'
  | 'results_range'
  | 'role'
  | 'rows_per_page'
  | 'save'
  | 'scope'
  | 'scope_email'
  | 'scope_mcp'
  | 'scope_offline_access'
  | 'scope_openid'
  | 'scope_profile'
  | 'search'
  | 'search_tag'
  | 'select_file'
  | 'select_self'
  | 'select_target_board_first'
  | 'select_user'
  | 'send'
  | 'send_as_plan'
  | 'send_as_report'
  | 'server_info'
  | 'settings'
  | 'share_title'
  | 'share_url'
  | 'show'
  | 'show_archived'
  | 'signin'
  | 'signout'
  | 'skip_consent'
  | 'slack'
  | 'slack_allowed_groups'
  | 'slack_enable'
  | 'slack_notify_channel'
  | 'slack_notify_channel_none'
  | 'slack_notify_events'
  | 'slack_workspace'
  | 'sort_order'
  | 'start_time'
  | 'state_off'
  | 'state_on'
  | 'status'
  | 'status_backlog'
  | 'status_doing'
  | 'status_done'
  | 'status_done_recent'
  | 'status_todo'
  | 'table_empty'
  | 'tag_color'
  | 'tag_manage'
  | 'tags'
  | 'target_board'
  | 'task_management'
  | 'team'
  | 'terms_of_service'
  | 'theme_dark'
  | 'theme_light'
  | 'theme_select'
  | 'theme_system'
  | 'ticket'
  | 'ticket_count'
  | 'ticket_link_url'
  | 'ticket_links'
  | 'ticket_summary'
  | 'time_range'
  | 'timezone'
  | 'title'
  | 'today'
  | 'token_active'
  | 'token_endpoint_auth_method'
  | 'token_endpoint_auth_method_basic'
  | 'token_endpoint_auth_method_post'
  | 'token_endpoint_auth_method_unsupported'
  | 'token_expiration'
  | 'token_expired'
  | 'transfer_billable'
  | 'transfer_pool_usage'
  | 'trust_device'
  | 'twofa'
  | 'twofa_enable'
  | 'unarchive'
  | 'unassigned'
  | 'unlink'
  | 'update'
  | 'update_busy_time'
  | 'update_client'
  | 'update_group'
  | 'update_link'
  | 'update_member'
  | 'update_passkey'
  | 'update_tag'
  | 'update_ticket'
  | 'update_user'
  | 'updated_at'
  | 'uptime'
  | 'url'
  | 'usage_count'
  | 'user'
  | 'user_manage'
  | 'username'
  | 'version'
  | 'via'
  | 'wait_seconds'
  | 'weekday'
  | 'weekdays_only'
  | 'welcome'
  | 'widget_list'
  // for Message
  | 'msg_added_comment'
  | 'msg_added_oidc_client'
  | 'msg_added_passkey'
  | 'msg_added_passkey_description'
  | 'msg_added_target'
  | 'msg_agent_approver_desc'
  | 'msg_agent_custom_instruction_desc'
  | 'msg_agent_email_desc'
  | 'msg_agent_mode_desc'
  | 'msg_agent_no_approver'
  | 'msg_agent_rule_none'
  | 'msg_agent_runner_desc'
  | 'msg_agent_token_env'
  | 'msg_archive_board'
  | 'msg_avatar_desc'
  | 'msg_board_key_change'
  | 'msg_board_slack_notify_desc'
  | 'msg_calendar_share_desc'
  | 'msg_calendar_share_disabled'
  | 'msg_calendar_share_enabled'
  | 'msg_calendar_share_title_saved'
  | 'msg_calendar_share_unavailable'
  | 'msg_calendar_share_url_rotated'
  | 'msg_cannot_delete_last_admin'
  | 'msg_communication_error'
  | 'msg_confirm_archive_board'
  | 'msg_confirm_delete_board'
  | 'msg_confirm_delete_mcp_token'
  | 'msg_confirm_deletion'
  | 'msg_confirm_disable_share'
  | 'msg_confirm_revoke_consent'
  | 'msg_confirm_rotate_token'
  | 'msg_confirm_rotate_url'
  | 'msg_confirm_unarchive_board'
  | 'msg_consent_invalid'
  | 'msg_consent_note'
  | 'msg_consent_request'
  | 'msg_consent_unverified_client'
  | 'msg_delete_board'
  | 'msg_deleted_target'
  | 'msg_drop_image'
  | 'msg_duplicated_agent_handle'
  | 'msg_duplicated_board_key'
  | 'msg_duplicated_tag_name'
  | 'msg_duplicated_token_name'
  | 'msg_dynamic_oidc_clients'
  | 'msg_email_not_verified'
  | 'msg_enter_otp'
  | 'msg_feature_unavailable'
  | 'msg_google_account_allowed_groups_desc'
  | 'msg_google_account_connected'
  | 'msg_google_account_enable_desc'
  | 'msg_google_account_not_connected'
  | 'msg_group_assign_admin_only'
  | 'msg_image_upload_failed'
  | 'msg_invalid_email_or_password'
  | 'msg_invalid_operation'
  | 'msg_invalid_session'
  | 'msg_link_google_for_calendar'
  | 'msg_mcp_token_desc'
  | 'msg_mcp_token_env'
  | 'msg_mcp_token_limit'
  | 'msg_mention_hint'
  | 'msg_no_access'
  | 'msg_no_agent_approvals'
  | 'msg_no_agent_runs'
  | 'msg_no_approvable_agent'
  | 'msg_no_boards'
  | 'msg_no_command_runs'
  | 'msg_no_due_soon_tickets'
  | 'msg_no_matching_assignees'
  | 'msg_no_matching_tags'
  | 'msg_no_matching_users'
  | 'msg_no_mentions'
  | 'msg_no_recent_activity'
  | 'msg_no_tags'
  | 'msg_no_tickets'
  | 'msg_not_allowed_domain_in_google'
  | 'msg_notify_email_digest'
  | 'msg_notify_slack_unavailable'
  | 'msg_otp_sent'
  | 'msg_owner_required'
  | 'msg_passkey_failed'
  | 'msg_re_auth'
  | 'msg_saved'
  | 'msg_slack_allowed_groups_desc'
  | 'msg_slack_bot_unavailable'
  | 'msg_slack_channel_empty'
  | 'msg_slack_channel_failed'
  | 'msg_slack_connected'
  | 'msg_slack_email_must_match'
  | 'msg_slack_enable_desc'
  | 'msg_slack_not_connected'
  | 'msg_system_error'
  | 'msg_ticket_list_limit'
  | 'msg_token_once'
  | 'msg_too_many_requests'
  | 'msg_updated_target'
  | 'msg_user_not_exist'
  | 'msg_validation_error'
  | 'msg_webpush_blocked'
  | 'msg_webpush_desc'
  | 'msg_webpush_failed'
  | 'msg_webpush_ios_standalone'
  | 'msg_webpush_no_device'
  | 'msg_webpush_unavailable'
  | 'msg_webpush_unsupported'
  | 'msg_widget_load_failed'
  | 'msg_within_a_day'
  | 'msg_within_days'
  // for Mail
  | 'mail_digest_body'
  | 'mail_digest_item'
  | 'mail_digest_item_excerpt'
  | 'mail_digest_more'
  | 'mail_digest_subject'
  | 'mail_notify_body'
  | 'mail_notify_excerpt_body'
  | 'mail_otp_body'
  | 'mail_otp_subject'
  // for Notify(メール / Slack で共用する文面)
  | 'notify_msg_agent_run_finished'
  | 'notify_msg_mentioned'
  | 'notify_msg_mentioned_comment'
  | 'notify_msg_ticket_assigned'
  | 'notify_msg_ticket_assigned_channel'
  | 'notify_msg_ticket_completed'
  | 'notify_msg_ticket_created'
  // for Slack
  | 'slack_msg_open_ticket'

export type LocaleItemError =
  | '@required_field'
  | '@invalid_name'
  | '@invalid_email'
  | '@invalid_password'
  | '@invalid_image_type'
  | '@invalid_image_size'
  | '@invalid_description'
  | '@invalid_time_range'
  | '@select_weekday'
  | '@invalid_title'
  | '@invalid_content'
  | '@invalid_board_key'
  | '@reserved_board_key'
  | '@invalid_tag'
  | '@invalid_agent_handle'
  | '@invalid_daily_limit'
  | '@invalid_slack_channel'
  | '@invalid_webpush_subscription'
  | '@invalid_notify_setting'
  | '@invalid_command_input'
  | '@invalid_command_value'
  | '@invalid_github_url'
  | '@invalid_github_repo'
export const el = (item: LocaleItemError) => item

export type LocaleItem = LocaleItemBase | LocaleItemError
export type DefaultLocaleItems = Record<LocaleItem, string>
export type LocaleItems = Partial<Record<LocaleItem, string>>
