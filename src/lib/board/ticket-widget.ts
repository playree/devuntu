...
export const listMyTickets = async (userId: string) => {
  const tickets = await prisma.ticket.findMany({
    where: myTicketWhere(userId, await getAccessibleBoardIds(userId), OPEN_TICKET_STATUSES),
...
export const listDueSoonTickets = async (userId: string, tz: string = DEFAULT_TZ) => {
  const where = myTicketWhere(userId, await getAccessibleBoardIds(userId), OPEN_TICKET_STATUSES)
