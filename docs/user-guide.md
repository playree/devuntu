...
- [アカウント設定](#アカウント設定)
- [コマンドを実行する](#コマンドを実行する)
  - [コマンドの定義を画面から編集する(管理者)](#コマンドの定義を画面から編集する管理者)
- [AIとの連携](#aiとの連携)
...
**メニューに出るのは、自分が実行を許可されているコマンドがある場合だけ**。

1. `/commands` で実行したいコマンドの「実行」を押す
2. 選択肢を選ぶ。**自由入力の欄は無い**(選べる値は管理者が定義したものだけ)
3. 確認ダイアログで内容を確かめて実行
...
### コマンドの定義を画面から編集する(管理者)

管理者は `/admin/commands` でコマンドごとの有効化・許可グループ・表示順を設定する。
**有効化していないコマンドは管理者も実行できない。** 許可グループを指定しない場合は管理者のみ。

定義ファイルに `editable` が設定されている実行先では、コマンドの追加・編集・削除も
この画面から行える。編集はコマンド 1 件ぶんの YAML を書く形で、保存時に検証される。
**接続先(ホスト・ユーザー・鍵)は画面からは変えられない。**

- 編集の導線が出ない場合は、その実行先が編集を許可されていないか、
...
- コマンドを削除すると、そのコマンドの有効化と許可グループの設定も消える
