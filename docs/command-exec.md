...
有効化・許可グループ・表示順だけで、`commands` の編集も `target.editable: true` を
書いたファイルに限られる([画面から編集する](#画面から編集する)を参照)。

- 対象はディレクトリ直下の `*.yaml` / `*.yml` のみ。サブディレクトリの中と、`.` で始まる名前は読まない
- **1 ファイルに 1 実行先。** そのファイルに書いたコマンドはすべてこの実行先で動く
- 実行先IDとコマンドIDは**ディレクトリ全体で一意**にする。重複した場合は、
...
別の実行先として別のファイルに書く。第一階層が `host` ではなく `target` なのはこのため。
...
`target.editable: true` を書いたファイルは、`/admin/commands` から `commands` を
追加・編集・削除できる。編集はコマンド 1 件ぶんの YAML をその場で書く形で、保存時に
定義ファイルと同じスキーマで検証される。

**`target` はどのファイルでも画面から編集できない。** 接続先・ユーザー・鍵を画面から
増やせないということなので、この経路で広がる範囲は「既に鍵が通っている実行先で、
...
3. 操作しているのが管理者で、ログインから間もない(破壊的コマンドと同じ再認証を求める)
...
- コマンドを画面から削除すると、そのコマンドの**有効化と許可グループの設定も消える**。
  同じ ID で別のコマンドを作り直したときに昔の許可がそのまま効くのを避けるため。
  ID を変更する更新も同じ扱いになる
...
| やりたいこと                       | 定義の書き方                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------- |
| リモートサーバーで実行             | 対象の実行先ごとに定義ファイルを作る                                                          |
...
`target.editable: true` を書いたファイルでは、**管理者に限り `executable` そのものを
画面から書ける**ようになるので、この前提が管理者に対してだけ外れる。

つまり**管理者アカウントの侵害が、アプリの管理権限から「SSH 先での任意コード実行」へ昇格する。**
...
- **接続先が増えない。** `target`(ホスト・ユーザー・鍵)は画面から一切変えられないので、
  影響は「既に鍵が通っている実行先で、その鍵のユーザーにできること」に閉じる
...
| 追加したファイルが丸ごと出てこない | 実行先IDかコマンドIDが他のファイルと重複していないか(重複すると両方とも落ちる) |
| 「定義ディレクトリを読み込めない」 | マウント漏れか、`COMMAND_DEF_DIR` がファイルを指している                       |
| 実行先が「未準備」                 | 鍵か known_hosts が読めていない。マウントとパーミッション(600)を確認           |
...
| 定義の編集ボタンが出ない           | `target.editable` と、`COMMAND_DEF_DIR` が書き込み可でマウントされているか     |
