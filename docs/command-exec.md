...
- **1 ファイルに 1 ホスト。** そのファイルに書いたコマンドはすべてこのホストで動く
- ホストIDとコマンドIDは**ディレクトリ全体で一意**にする。重複した場合は、
...
| 追加したファイルが丸ごと出てこない | ホストIDかコマンドIDが他のファイルと重複していないか(重複すると両方とも落ちる) |
