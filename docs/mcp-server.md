...
- [AIエージェント用ユーザー](#aiエージェント用ユーザー)
  ...
  - [自動運用のツール](#自動運用のツール)
- [画像の添付](#画像の添付)
  - [ファイルを直接送る(推奨)](#ファイルを直接送る推奨)
  - [base64 で送る](#base64-で送る)
  - [制限](#制限)
  - [アップロードトークン](#アップロードトークン)
...
以下は Claude Code の例。Codex CLI から使う場合は
[ブラウザを使わずユーザーとして登録する場合](#ブラウザを使わずユーザーとして登録する場合)を参照。
...
エンドポイントは `/api/mcp` で共通だが、クライアントへ返す MCP サーバー名(`serverInfo.name`)は
