...
[定期実行](#定期実行)を参照。厳密な整合性が要る場合は `--maintenance` を付ける。
...
1. メンテナンスモードを ON にする
2. 対象DBへの他の接続が 0 になるまで最大60秒待つ(アプリは実行中のワーカーが終わってから接続を手放すため、
   0 になれば書き込みは止まっている)。上限まで残っていれば**取得せずに中断する**
3. DB → S3 の順に取得する
4. **成否に関わらず OFF に戻す**(Ctrl+C / SIGTERM で止めた場合も)。OFF に失敗した場合は非0で終了するので、手で解除する

- 開始前から ON だった場合は、運用者が入れた遮断なので ON にも OFF にもしない(接続の解放待ちだけ行う)
...
- `full-backup --maintenance` は取得の間だけ ON にし、**終われば成否に関わらず OFF に戻す**
  (開始前から ON だった場合は触らない)
...
利用の少ない時間帯を選ぶ、あるいは厳密な整合性が必要なら `--maintenance` を付けて
書き込みを止めてから両方を取得する([メンテナンスモード連動](#メンテナンスモード連動--maintenance))。
...
| `git_check_suite`        | 更新が保持期間より古い(GitHub 連携の CI 結果。マージ済み・放置された PR のもの) | 90日       |
