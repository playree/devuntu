...
`agent_run` のように**DM とチャネルの両方を持つイベント**もある。1 回の発生で両方へ展開されるので、
発火の記録は 1 行で済む。
...
| `ticket_assigned` | 担当者が実際に変わったとき。自分で自分を担当にした場合と、担当がエージェント用ユーザー(DM を読まない)の場合は送らない。作成時に担当者を付けるのも「指定された」ものとして扱う |
...
| Slack チャンネル | ボードを見ている人                       | `Board.slackChannelId`(未設定なら投稿しない) |

DM の宛先は配信直前に `notify-recipient.ts` が引く(`ticketRequesterIds()`)。エージェント用ユーザーは
DM を読まないので除き、チケットが削除済みなら辿れないので宛先は空になる。

入口は `enqueueAgentRunFinished()`(`src/lib/notify/notify-trigger.ts`)ただ 1 つ。キューへ投入するだけなので、
送信の失敗はエージェントの実行記録へ波及しない。

- チャンネル設定は `Board.slackChannelId`。`/boards/[id]/settings` の「Slack通知」で選ぶ。
  **null ならチャンネルへは投稿しないが、DM は送る**
