...
- [トリガーの発火](#トリガーの発火)
  - [発火の条件](#発火の条件)
  - [変更前の状態は追加の問い合わせ無しで取れる](#変更前の状態は追加の問い合わせ無しで取れる)
  - [文面に出すものは投入時に確定させる](#文面に出すものは投入時に確定させる)
...
`agent_run` / `ticket_assigned` のように**DM とチャネルの両方を持つイベント**もある。1 回の発生で
両方へ展開されるので、発火の記録は 1 行で済む。同じイベントでも本人宛の DM と第三者が読む
チャンネルでは文面が変わる(`notify-content.ts` の `NotifyAudience`)。
...
```
[トリガー発火]  notify-trigger.ts … チケット操作 / エージェントの実行記録から判断
      ↓ 1 行 INSERT(notify-enqueue.ts)
...
- トリガー側は 1 INSERT で済み、チケット操作のトランザクションを重くしない。
  **投入は業務更新と同じトランザクションで行う**(トリガーへ `tx` を渡す)ので、操作が
  ロールバックされた場合に通知だけが残ることも、コミット後に落ちて通知だけが消えることもない
...
3. **配信** : チャネルごとに掴んで送る。チャネル間は `Promise.allSettled` で並行、
   **チャネル内は逐次**(ワークスペース単位のバーストを避ける)。
   メールは**ユーザー単位でまとめて掴んで 1 通で送る**([まとめて1通にする](#まとめて1通にする))
4. **パージ** : 送信できた配信は行ごと削除する(送信の記録はログにある)。
   子が無くなった `done` のアウトボックスも削除し、`failed` は配信・アウトボックスとも
   `NOTIFY_FAILED_RETENTION_MS` だけ残す(展開できなかったアウトボックスは配信行を持たないため、
   `done` の条件では消えない)
...
メールの取り出しだけは件数ではなく**送る相手**で区切る。件数で切ると同じユーザーの通知が
バッチ境界で 2 通に割れてしまうため、まず待たせている順に相手を選び、その相手の期限到来ぶんを
全部掴む(`claimEmailDeliveries()` / `notify-queue.ts`)。
...
| イベント           | 条件                                                                                                                                                                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mention`          | 増えたメンションがあるとき。本文を編集し直すたびに同じ相手へ通知しないよう、**増えた分だけ**を宛先にする(差分は呼び出し元が取る)                                                                                                                      |
| `ticket_assigned`  | 担当者が実際に変わって、かつ新しい担当者がいるとき(外しただけでは知らせる相手も内容も無い)。作成時に担当者を付けるのも同じ扱い                                                                                                                        |
| `ticket_created`   | チケットを作成したとき。常に発火する(送るかはボードの設定次第)                                                                                                                                                                                        |
| `ticket_completed` | **完了レーンへ入った瞬間だけ**。完了のまま並べ替えても、完了から戻しても発火しない。最初から完了で作った場合は作成と併せて発火する。GitHub 連携の PR マージによる自動完了でも発火し、そのときは操作者の無いシステム由来の通知になる(文面に PR を出す) |
| `agent_run`        | 実行が終了したとき(成功 / 失敗 / スキップ)。開始時は通知しない                                                                                                                                                                                        |

`ticket_assigned` は DM とチャネルの両方を持つので、**DM を届ける相手がいない場合も発火する**。
自分で自分を担当にした場合と担当がエージェント用ユーザーの場合は DM の宛先を空にするだけで、
チャンネルへは「誰が担当になったか」を知らせる。

**操作した本人は宛先から外す**。判断は展開時(`notify-fanout.ts`)の 1 箇所にあるので、
規則で導いた宛先(依頼者など)にも同じように効く。

### 変更前の状態は追加の問い合わせ無しで取れる

`assertTicketAccess()` が返す `TicketAccess` に `assigneeId` / `status` が入っているので、
認可の問い合わせでそのまま読める。差分検出のために SELECT を増やす必要はない。

ステータスだけを変える経路(`updateTicketStatus` / かんばんの DnD)は通知に載せる表示IDや件名を
読んでいないので、`enqueueTicketMoved()` が**発火が決まってから引く**。並べ替えのたびに
SELECT が増えることはない。

### 文面に出すものは投入時に確定させる

操作した人の表示名・コメントの抜粋・チケットの表示IDと件名は、投入の時点でペイロードへ入れる。
配信は遅れて走るため、そのときにはコメントやチケットが消えていることもある。

- 件名は `[表示ID] チケットタイトル`。リンク先は `/t/<表示ID>`(コメント経由はコメントの位置までフラグメントを付ける)
- 文面は宛先ユーザーのロケールで組み立てる(チャンネル宛は既定ロケール固定)
- メールと Slack はキューの上では独立した配信行になる。片方のチャネルが失敗してももう片方は止まらない
- Slack は投入直後に届くが、**メールは最大 5 分遅れる**([まとめて1通にする](#まとめて1通にする))
...
メールだけは配信時刻を `NOTIFY_EMAIL_WINDOW_MS`(5 分)で区切った**次の境界へ丸める**
(`nextEmailWindowAt()` / `src/lib/notify/notify-schedule.ts`)。同じ区間に発生した通知は同じ時刻へ
寄るので、取り出しの時点でユーザー単位にまとめれば必ず 1 通になる。

個々の通知は最大 5 分遅れる代わりに、**ユーザーあたり 5 分に 1 通**が構造的に保証される。
「1 通目は即時、以降まとめる」方式は最終送信時刻の状態管理が必要になるので採らない。

| 件数   | 件名                                            | 本文                                                                           |
| ------ | ----------------------------------------------- | ------------------------------------------------------------------------------ |
| 1 件   | `[表示ID] チケット名`(本文を開かずに判別できる) | `mail_notify_body` / `mail_notify_excerpt_body`                                |
| 複数件 | `mail_digest_subject`(件数だけを示す)           | `mail_digest_body` に `mail_digest_item` / `mail_digest_item_excerpt` を並べる |

- 1 通に載せる項目は `MAX_DIGEST_ITEMS`(20)で頭打ちにし、超過分は `mail_digest_more` の 1 行へ畳む。
  通知が溜まったときに本文が際限なく伸びないようにするため
- 通知が停止できることの案内は本文に 1 回だけ出す(項目ごとには繰り返さない)
- まとめた 1 通は**成否も一括**で反映する。送れなかった場合は同じ組がそのまま再試行に回るので、
  一部だけが二重に届くことはない
- HTML テンプレートは作らない(プレーンテキストのみ)
...
`/account` の「通知設定」内にある「Webプッシュ通知」(`webpush.tsx`)から登録・解除する
...
- `pushsubscriptionchange` → 再購読して `/api/webpush/subscribe` へ報告する。
  **Service Worker から Server Action は呼べない**ため、このルートだけを別に用意している
  ...
  `event.oldSubscription` が無くて `applicationServerKey` を埋められない場合は
  `/api/webpush/key` から実行時の公開鍵を引く
...
`/boards/[id]/settings` の「チャネル通知」(`channel-notify.tsx`)で、**通知先チャンネル 1 つと
通知するイベントの ON/OFF** を選ぶ。保存先は `BoardNotifySetting`(`boardId` + `event` でユニーク)。

- **行が無いイベントは通知しない**オプトイン方式。通知先が未選択、またはイベントが 1 つも ON でない
  場合は行を全部消す
- テーブルはイベントごとに別のチャンネルを持てる形にしてあるが、画面からは有効なイベント全行へ
  同じチャンネルIDを書く。イベント別チャンネルが必要になってもマイグレーションが要らない
- 設定できるのはボードの `owner` と管理者(`assertBoardAccess(..., 'manage')`)。
  プライベートボードは他の構成変更と同じく `assertTeamBoard` で弾く
- 保存時に一覧と突き合わせ、含まれないIDは弾く(設定できたように見えて通知だけ届かない状態を作らない)
- 宛先は配信直前に `getBoardNotifyChannels()` が引くので、**設定を消した後に投入された分も送られない**
- 差分更新にせず総入れ替えにしているのは、部分更新の分岐を作らないため

`ticket_created` / `ticket_completed` はチャネル通知だけのイベントで、`UserNotifySetting` には出ない
(個人ごとに「このボードのチケット作成を知りたい」は表せないため)。
...
| 宛先             | 誰に届くか                               | 設定                               |
| ---------------- | ---------------------------------------- | ---------------------------------- |
| DM               | **チケットの作成者**(処理を依頼した本人) | `/account` の通知設定(`agent_run`) |
| Slack チャンネル | ボードを見ている人                       | ボードのチャネル通知(`agent_run`)  |

**どちらの宛先も規則で導けるので、配信直前に `notify-recipient.ts` が引く。**
DM は `ticketRequesterIds()`(エージェント用ユーザーは DM を読まないので除き、チケットが削除済みなら空)、
チャンネルは `getBoardNotifyChannels()`。トリガー側は宛先を渡さない。

入口は `enqueueAgentRunFinished()`(`src/lib/notify/notify-trigger.ts`)ただ 1 つ。キューへ投入するだけなので、
送信の失敗はエージェントの実行記録へ波及しない。
...
- **チャンネル宛はロケールを解決する相手がいない**ので、文面は既定ロケール(`DEFAULT_LOCALE`)に固定する。
  DM は宛先ユーザーのロケールで組み立てる
- チャンネル通知は `UserNotifySetting` のオプトインとは独立している(チャンネルの購読者を個人設定では
  表せないため)。ただし管理者が `/admin/settings` で Slack 連携を無効にすれば止まる
  (判定は展開時に行うので、無効化した後に投入された分も送られない)

### 通知先チャンネルの一覧

ボードのチャネル通知の選択肢は `listSlackChannels()`(`src/lib/slack/slack-server.ts`)が
`users.conversations` で取得する。
...
- スコープは `channels:read` / `groups:read`(不足していれば `missing_scope` が返る)
...
- 貼った本人が利用停止中(`banned`)なら展開しない
- `getTicketAccess()` で閲覧権限を確認する。見えないチケットは展開せず URL のまま残す(未存在と権限不足は区別しない)
- プライベートボードのチケットは本人に見えていても展開しない(カードはチャンネルの全員に見えるため)
...
3. `NOTIFY_PAYLOAD_SCHEMA`(`notify-payload.ts`)・文面(`notify-content.ts`)・宛先の決め方
   ...
   **定義漏れはコンパイルエラーになる**。DM とチャネルで文面が変わるイベントは
   `NotifyAudience` で出し分ける
4. ロケールキー(`notify_event_*` / `notify_msg_*`)を `src/locale/index.ts` と ja / en へ追加する
5. `notify-trigger.ts` の発火条件を足す。**呼び出し元(Server Action / MCP)は触らない**
   ── 触る必要が出たら、判断が呼び出し元へ漏れている
...
- 配信は `notify-<チャネル>.ts` を作り、結果を `DeliveryOutcome`(`notify-outcome.ts`)へ落とす。
  分類を揃えることで再試行の判断は `notify-queue.ts` の 1 箇所で済む
- 1 tick の流れへの組み込みは `notify-dispatch.ts` の `DELIVERERS` へ足す
  (`Record<NotifyChannel, …>` なので追加漏れはコンパイルエラーになる)

**置き場所**

`src/lib/notify/notify.ts` はクライアントからも import されるため、サーバー専用の処理は置かない
(設定の読み書きは `notify-setting.ts`、投入は `notify-enqueue.ts`、キューの出し入れは `notify-queue.ts`、
1 tick の流れは `notify-dispatch.ts`、送信は各チャネルのファイル)。
