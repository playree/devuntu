...
`COMMAND_DEF_DIR` の直下に置く YAML が実行できる処理の定義そのもの。**1 ファイルに 1 実行先**を書き、
実行先ごとにファイルを分ける。接続先(`target`)は画面から作ることも変えることもできず、
`commands` の編集も `target.editable: true` を書いたファイルに限られる
