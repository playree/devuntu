...
| 変数名                  | 説明                               | 必須 | デフォルト                |
| ----------------------- | ---------------------------------- | ---- | ------------------------- |
| `MAINTENANCE_MODE_FILE` | メンテナンスモードのフラグファイル |      | `/app/config/maintenance` |
...
既定値はコンテナ内のパス。`compose.yaml` がホストの `./config` を `/app/config` へマウントしているため、
`tools` 側の既定(cwd 相対の `config/maintenance`)と同じファイルを指す。置き場を変える場合は
