...
- [稼働条件](#稼働条件)
  - [設定する画面](#設定する画面)
- [認証](#認証)
- [ランナー(devuntu_agent.py)](#ランナーdevuntu_agentpy)
  - [トークンの渡り方](#トークンの渡り方)
- [セットアップ](#セットアップ)
...
ボードの条件もある。**アーカイブ済みのボードと、エージェント自身がメンバー(直接 / グループ経由)でないボード**の
チケットは、担当・`agentMode` が揃っていても拾わない(エージェントはそのチケットを読み書きできないため)。

承認は「そのエージェントに任せる」判断なので、**担当を付け替えると `agentMode` / `agentState` は消える**
(付け替え先が人間・別のエージェント・未割り当てのいずれでも。Web / MCP 共通で `src/lib/board/ticket-mutation.ts` の `updateTicket` が行う)。
別のエージェントに任せる場合は、そのエージェントの承認者が改めて選び直す。
...
`AgentRunner.rule`(Markdown)は、そのエージェントが処理全体を通じて従う指示。
...
| `daily_limit`   | 1日に開始できる実行の上限に達した          |

時間帯は 0:00 からの分で持ち、開始と終了のどちらかが未指定なら終日。
**開始 > 終了は日跨ぎ**(例 22:00〜06:00 = 夜間のみ)。終了時刻ちょうどは含めない。
判定に使うタイムゾーンは `AgentRunner.timezone`(未設定なら `DEFAULT_TIMEZONE`)。

`dailyRunLimit` は 1日に開始できる実行の上限で、`0` は無制限。カウントをリセットする時刻は
`dailyResetMin`(既定 5:00)で、時間帯と同じタイムゾーンで判定する。上限のチェックは
`/api/agent/status` と実行の記録(`POST /api/agent/runs`)の両方で行う。
後者はトランザクションの中で数え直すので、同時に届いたリクエストが揃ってすり抜けることはない。

### 設定する画面

管理者は `/admin/agents/[id]`、承認者は `/agents` から同じ設定を操作できる。
どちらの画面も「自動運用」(稼働の有効/無効・ポーリング間隔・タイムゾーン・稼働時間帯・
1日の上限とリセット時刻)、「ルール」、「実行履歴」の3つを持つ。
実行履歴は保持期間(`AGENT_RUN_RETENTION_DAYS`)と件数の上限(`AGENT_RUN_KEEP`)を超えると
自動メンテナンスが消す([operations.md](operations.md#自動メンテナンス)を参照)。
最終ポーリング時刻・ホスト名・ランナーのバージョンはランナーの自己申告で、
稼働状況の表示にだけ使い認可には使わない。
...
- `workdir` は特定のリポジトリではなく、必要なリポジトリをその配下に clone して使う基点ディレクトリ。
  どのリポジトリを対象にするかはチケット本文やルール(`AgentRunner.rule`)の指示からエージェントが判断する。
...
| チケットが拾われない                         | 担当がエージェントか / チケットの「エージェント」が「任せない」でないか / ステータスが完了でないか / ボードがアーカイブ済みでなく、エージェントがボードのメンバーか                                                                         |
